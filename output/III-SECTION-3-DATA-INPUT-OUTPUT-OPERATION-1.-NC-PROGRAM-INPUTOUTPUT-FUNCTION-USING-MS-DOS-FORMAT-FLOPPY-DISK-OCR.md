...
## NC Program Input/Output Function Using MS-DOS Format Floppy
...
Program input and output using this function is possible with the following MS-DOS formats:
...
Even if a 3.5-inch floppy disk can be read and written to at an MS-DOS-compatible
...
In the program operation mode, part programs and other NC data stored in the NC memory are

copied to a floppy disk using the copy command.

(2)

In the program operation mode, part programs and other NC data saved in the floppy disk are
...
The reverse slash "\" is used as the delimiter between directory names (the "\'

following the device name can be omitted).

2.

It is not normally necessary to specify a device name when handling NC files but it
...
The total length of the sequence "device name:path-name\file name" must not
...
The length of the path-name must not exceed 47 characters.

5.

The file name consists of a main file name (with a maximum length of 8 characters)
...
Displays an MS-DOS format directory.

COPY

Copying*

Copies files from MS-DOS format to OSP format and vice versa.

RENAME

Renaming*

Used to change specified file names in the MS-DOS format.

DELETE

Used to delete specified files in the MS-DOS format.

Deletion*

FREE

Indicates the remaining memory capacity in the MS-DOS format.
...
Prohibits updating the information of specified files in the MS-
...
Program Input Work program files are input from the MS-DOS

formatted floppy disk to the memory disk while deleting any "%"
...
Work program files are output from the memory disk to an MS-

DOS formatted floppy disk. If option "E" is selected, only the "%"
...
Press the EDIT AUX mode selection key to select the PROG OPERATION
...
Press the function key [FB] (EXTEND) twice.


# DATE I DIR I PIP I EDIT I ~~Tl I LIST lcoNDENS I [EXTEND]
...
Enter the device name following ">DI" for the device which stores the files.
...
The use of the wild cards"*" and"?" is only valid for files; an error will occur if either

of these symbols is used with a device name or directory name (path name).

2.

A maximum of 12 file names and directory names can be displayed on each
...
"<DIR>" displayed in the sector column indicates that the entry is a directory.

4.

If a file name includes a character other than those indicated below, such a
...
The following options can be specified after the file name. They must be preceded
...
Press function key [F2] (COPY).


## DIR I COPY I RENAME I DELETE I FREE I
...
INPUT THE DEVICE NAME 00 FILE NAIIE, THEN PRESS
...
Enter the device name, path name, and file name of the program to be copied.
...
The COPY function can only be used to copy between the OSP format and
...
This function has no default device name and it is therefore essential to specify the

device name.

3.

If no destination file name is specified it is made the same as the source file name.

4.

Contiguous OSP format files cannot be overwritten. If an attempt is made to do this

the message "file attribute unsame" is displayed.

If the specified destination file name already exists, the message "file exist
...
If the copied file name contains any characters other than those listed below, these
...
TO DISPLAY OTHER INDEXES, AFTER PRESSING

[Fl] ,

INPUT THE DEV I CE NAME ANO FI LE NAME, THEN PRESS
...
Enter the file name (including the device name and path name) of the MS-DOS format file
...
If the specified file (currentfile name) does not exist in the floppy disk, the message

"no file" is displayed on the console lines and the renaming operation is terminated.

2.

If a file with the same name as that specified for the file after the change already
...
Specify only the file name (with no device name or path name) for the file name

after the change. An error will occur if a device name or path name is specified.

5.

If the specified file is a directory, the message "directory" is displayed on the
...
Enter the file name (including the device name and path name) of the MS-DOS format file that
...
The wild cards "*" and "?" can be used in the file name (wild cards cannot be used

1.

in path names).

If no option is specified deletion is executed unconditionally (unless rt is a path
...
Files protected by the file protection function cannot be deleted.

5.

The following option can be specified. It must be preceded by a semicolon ";".
...
Never specify any more than a device name in the command.
...
INPUT THE DEVICE NAME ANO FILE NAME, MN PRESS

[WRITE] KEY.


#### DEFAULT DEV I CE NAA£ =
...
Enter the file name (including the device name and path name).
...
The wild cards "*" and "?" can be used in file names.
...
If the option V (;V) is not specified, files will be protected (or have their protection
...
The following options can be specified. They must be preceded by a semicolon";".
...
Press function key [F1] (PROGRAM INPUT)
...
INPUT 11-lE DEVICE NAME AND FILE NAME, THEN PRESS
...
Enter the device name, path name, and file name for the work program which is to be input.
...
Press function key [F2] {PROGRAM OUTPUT)
...
TO DISPLAY OTHER lrflEXES, AFTER PRESSING


#### I NP\JT THE DEV I CE NAME AND FI LE NAME, THEN PRESS
...
Enter the device name, path name, and file name for the work program which is to be output.
...
Treatment of Non-ASCII Characters etc. by NC
...
If the destination file name in a copying operation already exists, the existing file is normally
...
this message can sometimes be displayed even when the file is successfully

overwritten.

(2)

When copying a file from the MS-DOS format to the NC, if the file name specified in the NC is
...
The floppy disk used with the MS-DOS format 1/0 function must be MS-DOS formatted.
...
Specifications such as "*A.MIN", where the wild card is used as the first character, are treated
