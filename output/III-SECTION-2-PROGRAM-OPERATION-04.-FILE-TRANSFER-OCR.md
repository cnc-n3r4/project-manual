...
Punches out the part program stored in the memory.
...
Copies the program file on memory.

Copy

Tape reader rapid forward feed motion
...
Set the program tape in the tape reader.
...
TO DISPLAY OTHER ltf)EXES, AFTER PRESSING
...
Enter the file name of the file to be punch and press the WRITE key.
...
Tape rewinding with or withouttape verification is selectable by setting bits 4 and 5 of NC
...
Press function key [F7] {PIP QUIT).
...
If an error occurs during tape reading, 31 characters preceding the error character are
...
TO DISPLAY OTliER INDEXES, AFTER PRESSING
...
TO DISPLAY OTHER l~ES. AFTER PRESSING
...
Press function key [F7] (PIP QUJTI,
...
That tape punching speed will be slowed down while machine operation is carried
...
600 tape feed holes are punched in the tape leader section.
...
The file name is punched out following the "$" code. (Program data is punched out in the ISO

coding system.)

(3)

50 tape feed holes are punched out.
...
"%", and "CR", "LF" codes are punched out.

(5)

The part program data is punched out following the program name (number).

(6)

The same number of tape feed holes as in (1) are punched out in the tape trailing section.

1.

When the program data is punched out ln the EIA coding system, the "CR" code is
...
The part program is split and punched out, if it is too long to be contained in one
...
# Tape length foe parameter seW~ I = I

• • • • , , $A.MIN • • • • • • % ® ~**** \, \ .... __ ®_®_• •..,..•-••.,..•_.I 1st tape
...
Set the tape to be verified in the tape reader in the same manner as for storing a program from
...
ltf)IJT THE DEVICE NAME AND FILE NAME. THEN PRESS
...
Enter the file name of the file to be verify and press the WRITE key.
...
INPUT THE DEVICE NAME ANO FILE NAt.£, THEN PRESS
...
There are two tape coding systems, EIA and ISO. The selection of a coding system can be
...
There are two different methods to operate the machine with stored programs.
...
To store program data following the program data already stored in the memory, follow the
...
When the file name is already registered in the memory and when it is necessary to store a
...
Type uy» and press the WRITE key.
...
If data does not need to be stored, type "N" and press the WRITE key.
...
File names can be specified and changed as required by inputting the following.
...
Stored in succession with a file name
...
The following command reads a tape which contains wrong codes up to the end while replacing
...
Reading of a Tape which has a File Name Punched in the EIA Code
...
Enter the file name of the file to be copy and press the WRITE key.

Example:

BOX-1350.MIN, BOX-2000.MIN
...
When the specified file name "BOX-1350.MIN" is not found in the memory, the
...
When the file name "BOX-2000.MIN" which has been specified as the output file
...
The output file name can be omitted when the output file name is the same as the

input file name.

4.

When the output file name is omitted, symbols "*" and "?" can be used in an input

file name. In this case, all the corresponding files are duplicated. (Refer to Section

5, 3. "DIRECTORY".)

In addition to the above duplicating functions, the following functions are optionally
